...
- Landing surface: when `isMainLanding` is true (apex `topiqu.com`, non-app route, no resolved `clientSite`), `app.vue` renders `<Landing />` in-app on every environment — the marketing site lives on the apex itself, never on a subdomain. (Previously the apex SSR-redirected to `landing.topiqu.com`; that redirect was removed so unauthenticated visitors stay on `topiqu.com`.) `/admin`, `/autorizace` and `/oauth-start` are excluded; the dev `useDevView` override still forces the surface on `localhost`.
