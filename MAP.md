...
- **AI:** Vercel `ai` SDK with `@ai-sdk/xai` (`grok-4-1-fast`) + Vue bindings — article generation, sentiment, and per-language article translation (§6 → *Article Translations*). Manual generation **streams**: `server/utils/ai/article.ts` splits into `buildArticleConfig` (prisma prefs + system prompt + `articleSchema`) / `finalizeArticle` (Unsplash/`[[IMAGE]]`+`[[POLL]]` swap, cover image) / `generateArticle` (blocking, used by the `generate-article` cron) / `streamArticle` (`streamObject`). `POST /api/articles/generate` pipes `partialObjectStream` as **NDJSON** (`{type:'partial'|'final'|'error'}`); token accounting (`consumeClientTokens`) + image/poll finalization run server-side after the stream so a dropped client can't skip billing. Client reads it via `composables/useArticleGeneration.ts → streamGenerate` — live title/perex/content typing into TipTap (which `setContent`s on external `v-model` change), placeholders swapped for the finalized payload on the `final` message. Consumed by `pages/admin/editor/[id].vue` + `Article/Modal.vue`.
