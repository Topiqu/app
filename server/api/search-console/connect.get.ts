...
export default defineEventHandler(async (event) => {
  ...
  setCookie(event, 'gsc_oauth_state', state, { httpOnly: true, secure: !import.meta.dev, sameSite: 'lax', path: '/', maxAge: 600 })
  const params = new URLSearchParams({
    client_id: clientId, redirect_uri: searchConsoleRedirectUri(), response_type: 'code',
    scope: 'openid email https://www.googleapis.com/auth/webmasters.readonly', access_type: 'offline',
    prompt: 'consent', include_granted_scopes: 'true', state,
  })
