export default defineEventHandler(async (event) => {
  ...
  await logAction({
    action: 'TENANT_INVITATION_RESENT',
    userId: user.id,
    clientSiteId: membership.clientSiteId,
    ip: getIp(event),
    metadata: { invitationId: id, email: invitation.email, expiresAt: expiresAt.toISOString() },
  })
  ...
  await sendEmail({
    ...
    data: {
      ...
      scopeCount: String(invitation.scopes.length),
      expirationDays: '7',
    },
  })
  return { expiresAt }
