export default defineEventHandler(async (event) => {
  ...
  if (!dbUser) throw createError({ statusCode: 404, message: t('common.errors.userNotFound')! })
  ...
  const bansCount = dbUser.bans.length

  if (dbUser.emailVerified) {
    score += weights.email
    checks.push({ label: 'profile.checks.emailVerified', ok: true })
  } else checks.push({ label: 'profile.checks.emailNotVerified', ok: false })

  if (dbUser.totpSecret) {
    score += weights.totp
    checks.push({ label: 'profile.checks.2faEnabled', ok: true })
  } else checks.push({ label: 'profile.checks.2faDisabled', ok: false })

  const metadata = logs[0]?.metadata
    ? typeof logs[0].metadata === 'string'
  ...
  if (dbUser.sessions.length === 0) checks.push({ label: 'profile.checks.noSessions', ok: false })
