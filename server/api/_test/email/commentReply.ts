...
export default defineEventHandler(async (event) => {
  ...
  const data = {
    userName: user.name,
    parentUsername: 'Typeček',
    commentContent: 'Můj obsah'.slice(0, 50) + '...',
    parentContent: 'Můj obsah'.slice(0, 50) + '...',
    commentUrl: commentUrl('123'),
    replyUrl,
    avatarUrl: user.avatarUrl || 'https://via.placeholder.com/50',
    logoUrl,
    unsubscribeUrl: `${base}/unsubscribe?email=${encodeURIComponent('randomassemail')}`,
  }
