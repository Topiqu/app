export interface SearchPerformance { page: string; query: string; clicks: number; impressions: number; ctr: number; position: number }
export type OpportunityType = 'STRIKING_DISTANCE' | 'LOW_CTR'

export const detectSearchOpportunities = (rows: SearchPerformance[]) => rows.flatMap((row) => {
  ...
  if (row.impressions >= 100 && row.position <= 10 && row.ctr < 0.02)
    opportunities.push({ type: 'LOW_CTR', score: Math.min(100, Math.round(Math.log10(row.impressions + 1) * 25)), reason: row })
