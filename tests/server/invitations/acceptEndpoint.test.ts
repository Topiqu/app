...
const invitation = {
  id: 'invite-1', clientSiteId: 'tenant-1', invitedById: 'owner-1', email: 'member@example.com',
  scopes: ['ARTICLE_WRITE'], expiresAt: new Date(Date.now() + 60_000), acceptedAt: null, revokedAt: null,
}
...
describe('POST /api/invitations/:token', () => {
  ...
  it.each(['credentials', 'oauth'])('accepts the invitation after a verified %s sign-in', async () => {
    const { handler, transaction } = await loadHandler({ id: 'member-1', email: 'Member@Example.com', sessionId: 'session-1' })
  ...
  it('rejects a signed-in account with a different email before writing membership', async () => {
    const { handler, transaction } = await loadHandler({ id: 'wrong-user', email: 'other@example.com', sessionId: 'session-2' })
    await expect(handler({} as never)).rejects.toMatchObject({ statusCode: 403, data: { code: 'INVITATION_EMAIL_MISMATCH' } })
