...
describe('systemic UX implementation contracts', () => {
  ...
  it('keeps the regression fixes for publication, comments, admin table, profile navigation and rate limiting', () => {
    ...
    expect(comments).toContain('<template #trailing><GifSelector')
    expect(comments).toContain("base: 'min-h-[100px] pe-14'")
    expect(comments).not.toContain('class="min-h-[100px] w-full"')
    expect(comments).not.toContain('absolute right-2 top-2')
