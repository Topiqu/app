...
describe('dashboard UI polish contracts', () => {
  ...
  it('uses current controls for poll editing and exposes the avatar file action', () => {
    ...
    expect(poll).toContain('rounded-(--topiqu-surface-radius)')
    expect(poll).toContain('border-default bg-default')
    expect(poll).not.toContain('bg-white')
    expect(poll).not.toContain('dark:bg-slate')
    const publicPoll = source('app/components/Article/Poll.vue')
    expect(publicPoll).toContain('rounded-(--topiqu-surface-radius)')
    expect(publicPoll).toContain('border-default bg-default')
    expect(publicPoll).toContain('role="heading" aria-level="4"')
    expect(publicPoll).not.toContain('<h4')
