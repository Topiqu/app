...
Manual article generation reserves an independent per-run budget derived by `articleGenerationReservation`: 24,000 provider-token credits without research, or 38,000 / 40,000 / 43,000 for quick / standard / deep research, plus 500 when YouTube lookup is selected, all converted through the active `TOKEN_RATIO`. The figures cover the writer, the worst normal verification/revision path and input headroom. This closes the old fixed-10k under-reservation without locking the tenant's whole wallet: concurrent authors can start when the remaining unreserved balance covers each run. Settlement charges exact usage up to that operation's reserved risk cap and releases the remainder. Fixed reservations in credit units: translation and SEO autopilot 5,000; community insights 1,500; prompt enhancement 1,000; sentiment 500. Billing records ratio, raw usage where available, and pricing version `2026-09-wallet-v1`. Manual generation stamps the operation with the options that drove its cost (`format`, `modules`, `researchDepth`) and the model behind each stage, on started, completed and aborted audit paths. Changing conversion rules requires a new price version.
