<template>
  <UContainer class="space-y-8 py-8 sm:py-10">
    ...
    <template v-else-if="data">
            ...
            <table class="w-full min-w-[36rem] text-left text-sm">
              <thead class="text-xs text-muted">
                <tr class="border-b border-default">
                  <th class="px-5 py-3 font-medium">{{ $t('visibility.crawlers.bot') }}</th>
                  <th class="px-4 py-3 font-medium">{{ $t('visibility.crawlers.kind') }}</th>
                  <th class="px-4 py-3 text-right font-medium">{{ $t('visibility.crawlers.requests') }}</th>
                  <th class="px-4 py-3 text-right font-medium">{{ $t('visibility.crawlers.pages') }}</th>
                  <th class="px-5 py-3 text-right font-medium">{{ $t('visibility.crawlers.lastSeen') }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="bot in data.crawlers.byBot" :key="bot.bot" class="border-b border-default last:border-0">
                  <td class="px-5 py-3 font-medium text-highlighted">{{ bot.bot }}</td>
                  <td class="px-4 py-3">
                    <UBadge color="neutral" variant="subtle">{{ $t(`visibility.crawlers.kinds.${bot.kind}`) }}</UBadge>
                  </td>
                  <td class="px-4 py-3 text-right tabular-nums">{{ number(bot.requests) }}</td>
                  <td class="px-4 py-3 text-right tabular-nums">{{ number(bot.pages) }}</td>
                  <td class="px-5 py-3 text-right text-muted"><NuxtTime :datetime="bot.lastSeenAt" relative /></td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <section class="rounded-(--topiqu-surface-radius) border border-default bg-default">
        <div class="flex flex-col gap-4 border-b border-default px-5 py-5 lg:flex-row lg:items-end lg:justify-between">
          <div class="max-w-2xl">
        ...
        <form
          class="grid gap-3 border-b border-default bg-elevated/40 p-5 sm:grid-cols-[minmax(0,1fr)_7rem_auto]"
        ...
        <div class="rounded-(--topiqu-surface-radius) border border-default bg-default">
          ...
          <div v-if="data.visibility.recentRuns.length" class="divide-y divide-default">
            <UCollapsible v-for="run in data.visibility.recentRuns.slice(0, 10)" :key="run.id">
              <button
                type="button"
                class="flex w-full items-start gap-3 px-5 py-4 text-left hover:bg-elevated/50 focus-visible:outline-2 focus-visible:outline-primary"
              >
                <span
...
const removePrompt = (id: string) =>
  mutate(() => apiFetch(`/api/ai-visibility/prompts/${id}`, { method: 'DELETE' }))
