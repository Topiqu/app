<template>
  <span class="inline-flex min-w-0 max-w-full">
    <UPopover
      mode="hover"
      :content="{ side: 'top', align: 'start', sideOffset: 8, collisionPadding: 16 }"
      :ui="{
        content:
          'w-[min(20rem,calc(100vw-2rem))] rounded-(--topiqu-surface-radius) p-4 text-left shadow-xl motion-reduce:transition-none',
      }"
    >
      <UButton
        :to="authorPath"
