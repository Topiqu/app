<template>
  <div class="w-full h-full flex flex-col justify-between text-white relative overflow-hidden bg-[#0f172a]">
    <img v-if="backgroundImage" :src="backgroundImage" class="absolute inset-0 w-full h-full object-cover opacity-60" />
    ...
    <div class="relative z-10 w-full h-full flex flex-col justify-between p-16">
      <div class="flex items-center gap-4">
        <img v-if="siteLogo" :src="siteLogo" class="h-12 w-auto object-contain rounded-md" />
      ...
      <div class="flex items-center gap-6 mt-4">
        <div class="flex items-center gap-3 bg-white/10 px-4 py-2 rounded-full border border-white/10 backdrop-blur-md">
          <img v-if="authorImage" :src="authorImage" class="w-10 h-10 rounded-full border-2 border-white/50" />
          <div
            v-else
