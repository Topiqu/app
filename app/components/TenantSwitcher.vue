<template>
  <UFormField v-if="memberships.length > 1" :label="$t('common.members.title')">
    <USelect v-model="active" :items="options" valueKey="value" labelKey="label" />
  </UFormField>
</template>
...
const active = computed({
  get: () => auth.value?.user.clientSiteId ?? '',
  set: async (clientSiteId: string) => {
