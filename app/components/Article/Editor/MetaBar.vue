<template>
  <div class="custom-ui flex flex-wrap items-center gap-x-2 gap-y-1.5">
    <span
      v-for="tag in selectedTags"
      :key="tag.id"
      class="inline-flex items-center gap-1 h-6 pl-2.5 pr-1 rounded-full border border-gray-200 dark:border-gray-700 text-[11px] font-medium text-gray-600 dark:text-gray-300"
    >
      {{ tag.name }}
      <button
        type="button"
        class="w-4 h-4 flex items-center justify-center rounded-full bg-transparent! hover:bg-gray-200! dark:hover:bg-gray-700! text-gray-400 hover:text-red-500"
    ...
    <ArticleEditorPopover :label="$t('common.labels.tags')" icon="mdi:tag-plus-outline">
      <template #trigger>{{ selectedTags.length ? '' : $t('common.labels.tags') }}</template>
      ...
      <div class="flex flex-col gap-2 w-60">
        <input
          ...
          :aria-label="$t('articles.tags.selectExistingTag')"
        ...
        >
          ...
          />
          {{ $t('articles.tags.addButton') }} „{{ tagQuery.trim() }}“
        </button>
      </div>
    </ArticleEditorPopover>

    <span class="w-px h-4 bg-gray-200 dark:bg-gray-800" aria-hidden="true" />
...
const releaseLabel = computed(() =>
  releaseAt.value ? formatTime(releaseAt.value, 'shortDatetime') : t('common.labels.releaseDate'),
)
